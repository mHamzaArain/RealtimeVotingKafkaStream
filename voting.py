...
overall = dict(list(conf.items()) + list(conf2.items()))
consumer = Consumer(overall)
    ...
    try:
        while True:
            ...
            if msg is None:
                ...
                #     "vote": 1
                # }
                conf3 = {"voting_time": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), "vote": 1}
                vote = dict(list(voter.items()) + list(chosen_candidate.items()) + list(conf3.items()))
